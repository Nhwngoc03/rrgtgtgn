...
const ProductDetail: React.FC<ProductDetailProps> = ({ productId, onBack, isAuthenticated = false, onOpenLogin = () => {} }) => {
  ...
  // include main product.image as first option so it's always selectable
  const allImages = [product.image, ...(product.imageLeft || [])];

  // Name tag options (Ngọc provided) - choose one deterministically per product
  const nameTags = [
    'Hình dáng lạ',
    'Quả quá to/ nhỏ',
    'Vỏ có vết rám nắng nhẹ'
  ];
  const nameTag = (product as any).tag || nameTags[parseInt(product.id, 10) % nameTags.length];
    ...
    <div className="w-full bg-white pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between border-b border-gray-100 mb-8">
        ...
        <div className="flex gap-2">
           <button onClick={handleBack} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><ChevronRight className="size-4 rotate-180" /></button>
      ...
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
          ...
          <div className="flex flex-col gap-3 w-20">
            {allImages.map((url, i) => (
              <div 
                ...
                className={`aspect-square rounded-md overflow-hidden border-2 cursor-pointer transition-all ${selectedImageIndex === i ? 'border-orange-400 ring-2 ring-orange-200' : 'border-gray-100 hover:border-orange-300'}`}>
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              {product.tags && product.tags.length > 0 ? (
                <>
                  ...
                  {(showAllTags ? product.tags : product.tags.slice(0, 3)).map((t, i) => {
                    const colors = [
                    ...
                    return (
                      <span key={i} className={`text-xs font-semibold px-3 py-1 rounded-full transition-all hover:shadow-md hover:scale-105 ${colors[i % colors.length]}`}>{t}</span>
                    );
                  })}
                  </div>
                  {product.tags.length > 3 && (
                    <button onClick={() => setShowAllTags(s => !s)} className="text-xs text-gray-600 ml-4 px-2 py-1 hover:bg-gray-100 rounded transition-colors font-semibold">{showAllTags ? '▼ Thu gọn' : `+ ${product.tags.length - 3}`}</button>
                  )}
                 </>
               ) : (
                 <span className="text-xs bg-amber-100 text-amber-800 border border-amber-300 font-semibold px-3 py-1 rounded-full ml-4 hover:shadow-md transition-all">{nameTag}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 pt-12 border-t border-gray-100">
